...
#include "FeedBooksAtomDataParser.h"
#include "../../constants/XMLNamespace.h"

FeedBooksAtomDataParser::FeedBooksAtomDataParser(NetworkBookList &books) : myBooks(books) {
	myDeep = 0;
	myLastXmlTag = "";
}

bool FeedBooksAtomDataParser::processNamespaces() const {
	return true;
}

void FeedBooksAtomDataParser::namespaceListChangedHandler() {
	myDCPrefix.erase();
	...
	//std::cerr << "<NAMESPACECHANGE newDCPrefix=\"" << myDCPrefix << "\" />" << std::endl;
}
...
void FeedBooksAtomDataParser::startElementHandler(const char *tag, const char **attributes) {
	/*if (BOOK_TAG == tag) {
	...
	if (!myLastXmlTag.empty()) {
		std::cerr << ">" << std::endl;
	}
	
	for (int i = 0; i < myDeep; ++i) {
		std::cerr << " ";
	}
	++myDeep;
	std::cerr << "<" << tag;
	for (const char **ptr = attributes; *ptr != 0; ++ptr) {
		std::cerr << " " << *ptr;
		if (*(ptr + 1) != 0) {
			std::cerr << "=\"" << *++ptr << "\"";
		}
	}
	myLastXmlTag = std::string(tag);
}

void FeedBooksAtomDataParser::endElementHandler(const char *tag) {
	...
	if (myBuffer.length() > 80) {
		myBuffer = myBuffer.substr(0, 77) + "...";
	}
	
	--myDeep;
	if (myLastXmlTag != std::string(tag)) {
		for (int i = 0; i < myDeep; ++i) {
			std::cerr << " ";
		}
		if (!myBuffer.empty()) {
		...
		if (!myBuffer.empty()) {
			std::cerr << ">" << myBuffer << "</" << std::string(tag) << ">" << std::endl;
		} else {
			std::cerr << " />" << std::endl;
		}
	}
...
void FeedBooksAtomDataParser::characterDataHandler(const char *data, size_t len) {
	myBuffer.append(data, len);
	
	/*std::string buff(data, len);
	ZLStringUtil::stripWhiteSpaces(buff);
	if (buff.empty()) {
		return;
	}
	for (int i = 0; i < myDeep; ++i) {
